...
// WhatsApp uses the number without the + prefix
...
export default function StickyContact({ carTitle }: StickyContactProps) {
  const t = useTranslations("StickyContact");

  const waMessage = encodeURIComponent(
    t("whatsappMessage", { car: carTitle })
  );
  ...
  return (
    <div className="fixed bottom-6 right-6 z-40 flex flex-col gap-3">
      ...
      <a
        href={`tel:${PHONE}`}
        className="group flex h-14 w-14 items-center justify-center rounded-full bg-primary-600 text-white shadow-lg transition-all hover:scale-110 hover:shadow-xl"
